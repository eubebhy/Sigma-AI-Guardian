...
Package phân loại văn bản cục bộ. Input là `str` và `StrictLevel`; output là một
...
- `content_classifier(text, strict_level="mid")`: rule trước, local model fallback.
- `rule_based_classifier(text, strict_level="mid")`: chỉ rule engine.
- `local_ai_classifier(text, strict_level="mid")`: chỉ local model.

`StrictLevel` hợp lệ là `xlow`, `low`, `mid`, `strict`, `xstrict`.
