...
File path: `src/agent/platform/linux/key_listener_backend.py`.
...
class _InputDevice(Protocol):
    ...
    def close(self) -> None:
        """Đóng file descriptor của input device."""
        ...
...
def close() -> None:
    """Đóng các input device đã cache và xóa cache listener."""

    global _keyboards, _mice

    devices = [*_keyboards, *_mice]
    _keyboards = []
    _mice = []
    ...
    for device in devices:
        try:
            ...
            errors.append(error)
    if errors:
        raise ExceptionGroup("Key listener cleanup failed", errors)


__all__ = ["close", "get_num_lock_state", "listen_keys", "listen_mice"]
